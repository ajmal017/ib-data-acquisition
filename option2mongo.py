...
class newFifteenOnly(CustomWrapper):
    ...
    def requestNextTicker(self, numDays=2):
        """Scan through the data Ids and request the first thing that isn't finished yet."""
        for reqId in self.dataIds:
            if not self.historicalDone[reqId]:
                ticker = self.tickers[reqId]
                print("Requesting daily data for ", ticker, ", reqId: ", reqId)
                cont = dict2Contract(ticker, self.optContracts[ticker]["strike"], "CALL", self.optContracts[ticker], None)
                cont.lastTradeDateOrContractMonth = "20180720"
                self.reqHistoricalData(reqId, cont, "", durationDay(numDays), sizeMin(15), "MIDPOINT", 1, 1, False, [])
